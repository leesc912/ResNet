...
class custom_in_top_k_metrics(tf.keras.callbacks.Callback) :
    ...
    '''
    def __init__(self, val_generator, record_log, *arg, **kwargs) :
        super(custom_in_top_k_metrics, self).__init__()

        self.val_gen = val_generator
        self.record_log = record_log
        self.num_batches = len(val_generator)
        self.num_data = val_generator.get_num_data()
    ...
    def on_epoch_end(self, epoch, logs = {}) :
        ...
        for idx in range(self.num_batches) :
            ...
            val_X, y_true = self.val_gen[idx]

            # y_pred shape : (batch_size, num_categories) (after softmax activation)
            y_pred = self.model.predict_on_batch(val_X)
...
class RecordCallbacks(tf.keras.callbacks.Callback) :
    ...
    def on_epoch_end(self, epoch, logs = {}) :
        ...
        self.save_json_file(self.ckpt_info_file, 
            {'max_val_acc_ckpt' : self.max_acc_dict, 'min_val_loss_ckpt' : self.min_loss_dict, 'the_latest_ckpt' : self.the_latest_dict})
