...
def check_parameters(**kwargs) :
    ...
    if model_structure in ['original', 'pre'] :
        if model_structure == 'pre' :
            bottleneck = kwargs["bottleneck"]

            if bottleneck and (num_layers - 2) % 9 :
                raise Exception("bottleneck : {} and ({} - 2) % (3 * 3) != 0".format(bottleneck, num_layers))

            elif not bottleneck and (num_layers - 2) % 6 :
                raise Exception("bottleneck : {} and ({} - 2) % (2 * 3) != 0".format(bottleneck, num_layers))

        else :
            if (num_layers - 2) % 6 :
                raise Exception("({} - 2) % (2 * 3) != 0".format(num_layers))            

    elif model_structure == 'wide' : # B(3, 3) - basic wide model
        if (num_layers - 4) % 3 :
            raise Exception("({} - 4) % (2 * 3) != 0".format(num_layers))

    else :
        raise Exception("model structure : {} not in {}".format(model_structure, ['original', 'pre', 'wide']))
...
def load_checkpoint(**kwargs) :
    ...
    if isinstance(ckpt_model_path, list) and len(ckpt_model_path) == 1 :
        ckpt_model_path = ckpt_model_path[0]

    if not isinstance(ckpt_model_path, list) :
        assert os.path.exists(ckpt_model_path)
        assert load_type in ['min_loss_ckpt', 'max_acc_ckpt', 'the_latest_ckpt'] or isinstance(load_type, int)
