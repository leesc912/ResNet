...
def non_bottleneck_block(inputs, out_channels, size, strides, init, reg, momentum, 
    ...
    outputs = _Conv2D(
        bn_act_block(outputs, momentum), out_channels, size, 1, init, reg
    )
...
def make_preActivation_ResNet(inputs, **kwargs) :
    ...
    num_blocks      = (num_layers- 2) // 9 if bottleneck else (num_layers - 2) // 6
    l2_value        = 0.0001
    num_filters     = [e * 4 if bottleneck else e for e in [16, 32, 64]]
        ...
        for inner_idx in range(num_blocks) :
            if (outer_idx != 0) and (inner_idx == 0) :
                outputs = make_residual_block(inputs, in_channels, num_filters[outer_idx], 3,
                    2, k_init, l2(l2_value), momentum, bottleneck, True)
                ...
                outputs = make_residual_block(inputs, in_channels, num_filters[outer_idx], 3,
                    1, k_init, l2(l2_value), momentum, bottleneck, inner_idx == 0)
